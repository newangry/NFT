...
export default async function handler(
    ...
    const full_prompt = ` User query: '${query}'  You are Fomular Expert, so please answer only ${type} fomular or code based on User query. There is no need summary or explaination.`;
    const messages = [
        {
            role: 'user',
            content: full_prompt
